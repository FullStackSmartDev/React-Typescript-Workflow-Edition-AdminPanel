...
import { getZipCodeList } from "../../../../../../../mocks/ZipCodeMock";

export default class ZipCode extends React.Component<any, any> {
    ...
    addIconClick = (id:number) => {
        let selectedZipItem = this.state.zipCodeList.find(p=>p.id === id);
        if(selectedZipItem != undefined){
            selectedZipItem.isIconClick = true;
        }
        
        this.setState({zipCodeList: this.state.zipCodeList.map(el => (el.id === id ? {...el, selectedZipItem}:el))
        
        }, () =>{ this.props.selectedZipItemClick(this.state.zipCodeList.find(p=>p.id === id))});
    }

    subtractIconClick = (id:number) => {
        let selectedZipItem = this.state.zipCodeList.find(p=>p.id === id);
        if(selectedZipItem != undefined){
            selectedZipItem.isIconClick = false;
        }
    ...
    getAllZipCodeList = () => {
        ...
        if(this.props.selectedStateList != undefined && this.props.selectedCountyList != undefined ){
            
            zipList = this.props.selectedStateList.map((stateList,index) =>{  
                let ind = 0;              
                return(
                    this.props.selectedCountyList.map((countyList,index) => {
                        return (
                            <Fragment>
                        {
                            this.state.zipCodeList.filter(p=>p.isIconClick === false && p.stateId=== stateList.id && p.countyId === countyList.id).map((zipCodeList, index) => {
                                ind = ind + 1;
                                    return(
                                        <ul>
                                            {ind === 1 ?
                                            <li>{stateList.name}</li>
                                            :
                                            <li></li>
                                            }
                                            {ind === 1 ?
                                            <li>{countyList.name}</li>
                                            :
                                            <li></li>
                                            }
                                            <li>{zipCodeList.name}</li>
                                            <li>
                                                <svg onClick={() => this.addIconClick(zipCodeList.id)} width="16" height="15" viewBox="0 0 16 15" fill="none" xmlns="http://www.w3.org/2000/svg">
                ...
                return(
                    this.props.selectedCountyList.map((countyList,index) => {
                        return (
                            <Fragment>
                        {
                            this.state.zipCodeList.filter(p=>p.isIconClick === true && p.stateId=== stateList.id && p.countyId === countyList.id).map((zipCodeList, index) => {
                                ind = ind + 1;
                                    return(
                                        <ul>
                                            {ind === 1 ?
                                            ...
                                            <li>{zipCodeList.name}</li>
                                            <li>
                                            <svg onClick={() => this.subtractIconClick(zipCodeList.id)} width="15" height="15" viewBox="0 0 15 15" fill="none" xmlns="http://www.w3.org/2000/svg">
  ...
  render() {
      const zipCodeList = this.getAllZipCodeList();
      const selectedZipCodeList = this.getSelectedZipCodeList();
    return (
                ...
                <div className="states-container">
                    <Grid container>
                        ...
                        <div className="availbale-states scroll-bar">
                                ...
                                <div className="selected-available-heading">
                                    <ul><li>Selected States</li></ul>
                                    <ul><li>Selected Counties</li></ul>
                                    <ul><li>Available Zip Codes</li></ul>
                                    <ul><li></li></ul>
                                </div>
                                <div className="available-zip-code-list">
                                    {zipCodeList}
                                </div>
                            </div>
                        </Grid>
