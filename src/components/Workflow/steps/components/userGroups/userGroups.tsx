...
import { getUserGroupList } from "../../../../../mocks/grid/userGroups-mock";
...
export default class UserGroups extends React.Component<any, any> {
    ...
    addIconClick = (id: number) => {
        let selectedUserItem = this.state.userGroupList.find(p=>p.id === id);
        if(selectedUserItem != undefined){
            selectedUserItem.isIconClick = true;
        }
        
        this.setState({
            userGroupList: this.state.userGroupList.map(el => 
    ...
    addAllOnClick = () => {
        this.state.userGroupList.forEach(el => {
            el.isIconClick = true;
        })
        this.setState({
    ...
    subtractIconClick = (id: number) => {
        ...
        this.setState({userGroupList: this.state.userGroupList.map(el => (el.id === id ? {...el, selectedUserItem}:el))
    ...
    getAllRoleGroupList() {
        let userGroupList = {};
        userGroupList = this.state.userGroupList.filter(p => p.isIconClick === false).map((userGroupList, index) => {
            return (
                <ul>
                   <li>
                        <div className="top">{userGroupList.name}</div>
                    </li>
                    <li>
                        <svg onClick={() => this.addIconClick(userGroupList.id)} width="16" height="15" viewBox="0 0 16 15" fill="none" xmlns="http://www.w3.org/2000/svg">
    ...
    getSelectedRoleGroupList() {
        let userGroupList = {};
        userGroupList = this.state.userGroupList.filter(p => p.isIconClick === true).map((userGroupList, index) => {
            return (
                <ul>
                    <li>
                        <div className="top">{userGroupList.name}</div>
                    ...
                    <li>
                        <svg onClick={() => this.subtractIconClick(userGroupList.id)} width="15" height="15" viewBox="0 0 15 15" fill="none" xmlns="http://www.w3.org/2000/svg">
    ...
    render() {
        const userGroupList = this.getAllRoleGroupList();
        const selectedRoleGroupList = this.getSelectedRoleGroupList();
        return <>
            <div className="users-container">
                <Grid container>
                    <Grid item xs={6} className="add-grid">
                        <div className="heading-area">
                            <span className="heading">AVAILABLE USER GROUPS:</span>
                            <span className="search-input-group"><Input placeholder="Search..." prefix={<SearchSvg />} /></span>
                    ...
                    <Grid item xs={6} className="remove-grid">
                        <div className="heading-area">
                            <span className="heading">ADDED USER GROUPS:</span>
                            <span className="search-input-group"><Input placeholder="Search..." prefix={<SearchSvg />} /></span>
