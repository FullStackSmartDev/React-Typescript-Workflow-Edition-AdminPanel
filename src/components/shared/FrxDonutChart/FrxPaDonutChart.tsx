...
class FrxDonutChart extends Component<FrxDonutChartProps>{
    ...
    componentDidMount = () => {
        ...
        var __data = data.sort(function (a: any, b: any) { return a.key === 'open' ? 1 : -1 })
        ...
        arcs.append("path")
            .attr("fill", function (d: any, i: number) {
                return __data[i].key === 'open' ? "#59B35E" : "#666666" && __data[i].key === 'denied' ? "#666666" : "#F65A1C"
            })
            .each((d: any) => {
                d.outerRadius = __data[d.index].key === 'open' ? this.outerRadius : this.outerRadius + 11;
            })
            .attr("d", this.arc)
            .attr("d", (d) => {
                return this.arc(d);
    ...
    render() {
        return (<>
            <svg id="donut" height={220} width={230}>
            </svg>
        </>
        )
    };
