...
import { getDrugDetailsColumnMO, getDrugDetailsColumnNM } from "../DrugGridColumn";
...
class DrugDetailMOMN extends React.Component<any, any> {
  ...
  saveClickHandler = () => {
    ...
    if (this.state.selectedDrugs && this.state.selectedDrugs.length > 0) {
      ...
      if (this.state.activeTabIndex === 0) {
        ...
        this.rpSavePayload.monm_trademark = this.state.indicatorMM
        this.rpSavePayload.is_select_all = this.state.isSelectAll
        apiDetails["messageBody"] = this.rpSavePayload;

        apiDetails["pathParams"] = this.props?.formulary_id + "/" + this.state.lobCode + "/" + this.state.indicatorMM + "/" + momnConstants.TYPE_REPLACE;
        ...
        this.rmSavePayload.monm_trademark = this.state.indicatorMM
        this.rmSavePayload.is_select_all = this.state.isSelectAll
        apiDetails["messageBody"] = this.rmSavePayload;

        apiDetails["pathParams"] = this.props?.formulary_id + "/" + this.state.lobCode + "/" + this.state.indicatorMM + "/" + momnConstants.TYPE_REMOVE;
  ...
  getMOMNDrugsList = ({index = 0, limit = 10, listPayload = {}, searchBody = {}} = {}) => {
    if (this.state.indicatorMM) {
      let apiDetails = {};
      apiDetails["apiPart"] = momnConstants.GET_MONM_FORMULARY_DRUGS;
      apiDetails["pathParams"] = this.props?.formulary_id + "/" + getLobCode(this.props.formulary_lob_id) + "/" + this.state.indicatorMM;
      apiDetails["keyVals"] = [
        { key: momnConstants.KEY_ENTITY_ID, value: this.props?.formulary_id },
      ...
      this.props.getDrugDetailsMOList(apiDetails).then((json) => {
        ...
        var gridData = tmpData.map((el) => {
          ...
          if(this.state.activeTabIndex !== 2) {
            if(this.state.indicatorMM === "MO" && (element.is_mo && element.is_mo === "Y")) {
              gridItem["isChecked"] = true;
              gridItem["isDisabled"] = true;
              gridItem["rowStyle"] = "table-row--blue-font";
            } else if(this.state.indicatorMM === "NM" && (element.is_nm && element.is_nm === "Y")) {
              gridItem["isChecked"] = true;
              gridItem["isDisabled"] = true;
              gridItem["rowStyle"] = "table-row--blue-font";
            }
  ...
  onClickTab = (selectedTabIndex: number) => {
    ...
    this.setState({ tabs, activeTabIndex, selectedDrugs: [], showDG: false, isSelectAll: false });
  };
  ...
  onApplyFilterHandler = (filters) => {
    ...
    if (filters && filter.length > 0) {
      ...
      this.getMOMNDrugsList({ listPayload: this.listPayload });
    }
  }
  ...
  onApplySortHandler = (key, order, sortedInfo) => {
    ...
    if (this.props.advancedSearchBody) {
      this.getMOMNDrugsList({ searchBody: this.props.advancedSearchBody });
    } else {
      this.getMOMNDrugsList();
    }
  };
  ...
  componentWillReceiveProps(nextProps) {
    ...
    let monmStatus = this.state.indicatorMM ? this.state.indicatorMM : "MO";
    ...
    if (nextProps.advancedSearchBody && nextProps.populateGrid) {
      this.getMOMNDrugsList({ listPayload: this.listPayload, searchBody: nextProps.advancedSearchBody});
      let payload = {
  ...
  render() {
    ...
    let columns = this.state.indicatorMM === "MO" ? getDrugDetailsColumnMO() : getDrugDetailsColumnNM();
