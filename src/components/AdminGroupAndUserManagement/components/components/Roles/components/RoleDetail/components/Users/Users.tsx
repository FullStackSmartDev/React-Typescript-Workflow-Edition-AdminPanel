...
import { getUsersList } from "../../../../../../../../../mocks/grid/admin-roles-mock";
import "./Users.scss"
...
export default class Users extends React.Component<any, any> {
    ...
    addIconClick = (id: number) => {
        let selectedUserItem = this.state.usersList.find(p=>p.id === id);
        if(selectedUserItem != undefined){
            selectedUserItem.isIconClick = true;
        }
        
        this.setState({usersList: this.state.usersList.map(el => (el.id === id ? {...el, selectedUserItem}:el))
