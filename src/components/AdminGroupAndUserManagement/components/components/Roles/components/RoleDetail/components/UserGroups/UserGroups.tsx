...
import { getUserGroupsList } from "../../../../../../../../../mocks/grid/admin-roles-mock";
...
export default class UserGroups extends React.Component<any, any> {
    state = {
        userGroupsList: getUserGroupsList()
    }

    addIconClick = (id: number) => {
        let selectedUuserGroupsItem = this.state.userGroupsList.find(p=>p.id === id);
        if(selectedUuserGroupsItem != undefined){
            selectedUuserGroupsItem.isIconClick = true;
        }
        
        this.setState({userGroupsList: this.state.userGroupsList.map(el => (el.id === id ? {...el, selectedUuserGroupsItem}:el))
    ...
    subtractIconClick = (id: number) => {
        let selectedUuserGroupsItem = this.state.userGroupsList.find(p=>p.id === id);
        if(selectedUuserGroupsItem != undefined){
            selectedUuserGroupsItem.isIconClick = false;
        }
    ...
    getAllUserGroupsList() {
        let userGroupsList = {};
        userGroupsList = this.state.userGroupsList.filter(p => p.isIconClick === false).map((userGroupsList, index) => {
            return (
                <ul>
                    <li>
                        <div className="top">{userGroupsList.name}</div>
                        <div className="bottom">{userGroupsList.company}</div>
                    </li>
                    <li>
                    <EyeIcon />
                        <svg onClick={() => this.addIconClick(userGroupsList.id)} width="16" height="15" viewBox="0 0 16 15" fill="none" xmlns="http://www.w3.org/2000/svg">
    ...
    getSelectedUserGroupsList() {
        let userGroupsList = {};
        userGroupsList = this.state.userGroupsList.filter(p => p.isIconClick === true).map((userGroupsList, index) => {
            return (
                <ul>
                    <li>
                        <div className="top">{userGroupsList.name}</div>
                    ...
                    <li>
                    <EyeIcon />
                        <svg onClick={() => this.subtractIconClick(userGroupsList.id)} width="15" height="15" viewBox="0 0 15 15" fill="none" xmlns="http://www.w3.org/2000/svg">
    ...
    render() {
        const userGroupsList = this.getAllUserGroupsList();
        const selecteduserGroupsList = this.getSelectedUserGroupsList();
        return <>
