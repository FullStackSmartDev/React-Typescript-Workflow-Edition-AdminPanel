...
import { getRoleListData } from "../../../../../../../mocks/grid/admin-roles-mock";
import { RolesListGridColumns } from "../../../../../../../utils/grid/columns";
import DropDown from "../../../../../../shared/Frx-components/dropdown/DropDown";
import PanelHeader from "../../../../../../shared/Frx-components/panel-header/PanelHeader";
import FrxGridContainer from "../../../../../../shared/FrxGrid/FrxGridContainer";
import RoleUpdate from "../RoleDetail/components/RoleUpdate/RoleUpdate";
...
interface RoleProps {
    roleClick: boolean;
    roleClickEvent: any;
    addRoleClickEvent:any;
}

export default class RoleList extends React.Component<RoleProps, any> {
    constructor(props) {
        super(props);
    }
    state = {
        isRoleClick: false,
        roleListData: getRoleListData()
    };

    onRoleNameClick = (id: any) => {
        this.setState({ isRoleClick: true });
        this.props.roleClickEvent(true);
    };

    addRoleClick = () => {
        this.props.addRoleClickEvent(true);
    }
    render() {
        return (
            <Fragment>
                {(this.state.isRoleClick || this.props.roleClick) === false ? (

                    <div className="rolelist-grid-container">
                        <div className="grid-container-header-wrapper">
                            <div className="grid-name-fields">
                                <PanelHeader
                                    title="Role List"
                                    tooltip="Role List"
                                    className="rolelist-grid-panel-header"
                                />
                            </div>
                            <div className="field-container">
                                <DropDown
                                    className="rolelist-type-dropdown"
                                    placeholder="All"
                                    options={["All", "None"]}
                                />
                            </div>
                    ...
                    <Button className="add-new-role-btn" onClick={this.addRoleClick}>
                      + Add New Role
                    </Button>
                  </div>
                    ...
                    <Button className="add-new-customer-btn">
                      Add New Customer
                        ...
                        <div className="rolelist-grid-inner-container">
                            <FrxGridContainer
                                enableSearch
                                enableColumnDrag
                                onSearch={() => { }}
                                ...
                                columns={RolesListGridColumns({
                                    onRoleNameClick: (id: any) =>
                                        this.onRoleNameClick(id),
                                })}
                                data={this.state.roleListData}
                                expandable={{
                                    isExpandable: true,
                                    expandIconColumnIndex:
                                    RolesListGridColumns({}).length + 1,
                                    expandedRowRender: (props) => (
                                        <RoleUpdate expandModeInd={true} />
                                    ),
                                    expandCloseIcon: (
                                      <span>
                                        <svg
