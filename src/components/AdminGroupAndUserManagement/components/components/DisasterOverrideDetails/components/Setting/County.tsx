...
import { getCountyList } from "../../../../../../../mocks/CountyMock";


export default class County extends React.Component<any, any> {
    ...
    addIconClick = (id:number) => {
        let selectedCountyItem = this.state.countyList.find(p=>p.id === id);
        if(selectedCountyItem != undefined){
            selectedCountyItem.isIconClick = true;
        }
        
        this.setState({countyList: this.state.countyList.map(el => (el.id === id ? {...el, selectedCountyItem}:el))
        
        }, () =>{ this.props.selectedCountyItemClick(this.state.countyList.find(p=>p.id === id))});
    }

    subtractIconClick = (id:number) => {
        let selectedCountyItem = this.state.countyList.find(p=>p.id === id);
        if(selectedCountyItem != undefined){
            selectedCountyItem.isIconClick = false;
        }
    ...
    getAllCountyList =()=>{
        ...
        if(this.props.selectedStateList != undefined){
            
            countyList =  this.props.selectedStateList.map((stateList,index) =>{  
                let ind = 0;              
                return(
                    <Fragment>
                        {
                            this.state.countyList.filter(p=>p.isIconClick === false && p.stateId=== stateList.id).map((countyList, index) => {
                                ind = ind + 1;
                                if(countyList.stateId == stateList.id){
                                    return(
                                        <ul>
                                            {ind === 1 ?
                                            ...
                                            <li>
                                                <svg onClick={() => this.addIconClick(countyList.id)} width="16" height="15" viewBox="0 0 16 15" fill="none" xmlns="http://www.w3.org/2000/svg">
        ...
        if(this.props.selectedStateList != undefined){
            ...
            countyList =  this.props.selectedStateList.map((stateList,index) =>{  
                ...
                return(
                    <Fragment>
                        {
                            this.state.countyList.filter(p=>p.isIconClick === true).map((countyList, index) => {
                                ind = ind + 1;
                                if(countyList.stateId == stateList.id){
                                    return(
                                        <ul>
                                            ...
                                            <li>{countyList.name}</li>
                                            <li>
                                            <svg onClick={() => this.subtractIconClick(countyList.id)} width="15" height="15" viewBox="0 0 15 15" fill="none" xmlns="http://www.w3.org/2000/svg">
  ...
  render() {
    const countyList = this.getAllCountyList();
    const selectedCountyList = this.getSelectedCountyList();
    return (
                ...
                <div className="states-container">
                    <Grid container>
                        ...
                        <div className="availbale-states scroll-bar">
                                ...
                                <div className="available-county-list">
                                    {countyList}
                                </div>
                            </div>
                        </Grid>
