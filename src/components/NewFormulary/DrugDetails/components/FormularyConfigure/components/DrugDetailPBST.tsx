...
import { getDrugDetailsColumnPBST } from "../DrugGridColumn";
...
class DrugDetailPBST extends React.Component<any, any> {
      ...
      if (this.state.activeTabIndex === 0) {
        ...
        this.props.postRemovePBSTDrug(apiDetails).then((json) => {
          if (json.payload && json.payload.code && json.payload.code === "200") {
            ...
            this.getPBSTDrugsList({ listPayload: this.listPayload });
          } else {
            showMessage("Failure", "error");
          }
        });
  ...
  getPBSTDrugsList = ({index = 0, limit = 10, listPayload = {}, searchBody = {}} = {}) => {
    let apiDetails = {};
    ...
    this.props.getDrugDetailsPBSTList(apiDetails).then((json) => {
      ...
      var gridData = tmpData.map((el) => {
        ...
        if(this.state.activeTabIndex !== 2) {
          if(element.is_pbst && element.is_pbst === "Y") {
  ...
  componentWillReceiveProps(nextProps) {
    ...
    if (nextProps.advancedSearchBody && nextProps.populateGrid) {
      this.getPBSTDrugsList({ listPayload: this.listPayload, searchBody: nextProps.advancedSearchBody});
      let payload = {
  ...
  onApplySortHandler = (key, order, sortedInfo) => {
    ...
    if (this.props.advancedSearchBody) {
      this.getPBSTDrugsList({ searchBody: this.props.advancedSearchBody });
    } else {
      this.getPBSTDrugsList();
    }
  };
  ...
  render() {
    ...
    let columns = getDrugDetailsColumnPBST();
